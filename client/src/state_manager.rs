    ...
    C: WsClient<WsClientOutgoing, WsClientIncoming>,
{
    ...
    pub async fn diff_all(&self) -> Result<Vec<LocalOpLog>> {
        ...
        let paths = reader
            .walk(&self.client.vault_root)
            .await?
            .into_iter()
            .filter(|p| p.to_str().unwrap().ends_with(".md"));
    ...
    pub async fn diff_path(&self, abs_path: &PathBuf) -> Result<LocalOpLog> {
        ...
        let diff_path = self.diff_root_path().join(bin_path);
    ...
    fn apply_to_op_log(op_log: &mut OpLog, agent: AgentId, content: String) -> Result<()> {
        ...
        let delete_items = diff.iter().filter(|x| matches!(x.tag(), ChangeTag::Delete));

        let mut removed_indexes = vec![];
        for item in delete_items {
            let index = item.old_index().unwrap();

            let removed_offset = removed_indexes.iter().filter(|x| &index > x).count();
        ...
        let insert_items = diff.iter().filter(|x| matches!(x.tag(), ChangeTag::Insert));
...
mod tests {
    ...
    fn diffs_are_applied_correctly() {
        ...
        for case in cases {
            ...
            let mut op_log = OpLog::load_from(&*op_log.encode(EncodeOptions::default())).unwrap();

            let agent = op_log.get_or_create_agent_id("abc");

            let result = StateManager::<TestFsAdapter, TestWsClient>::apply_to_op_log(
                &mut op_log,
                agent,
                case.1.to_string(),
            );
